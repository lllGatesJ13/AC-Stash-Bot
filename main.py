...
# Flask setup (for UptimeRobot pings)
app = Flask(__name__)

@app.route("/")
def home():
    log.info("🌐 Ping received from Render")
...
try:
    ...
    """)
    conn.commit()
    log.info("✅ Database table 'users' ensured")
...
def is_valid_token(entry: dict) -> bool:
    return time.time() - entry.get("token_timestamp", 0) < 3600
...
async def auto_generate():
    try:
        ...
        log.info("⛏️ Mining tick complete")
    except Exception as e:
        log.error(f"auto_generate failed: {e}")
...
async def account(interaction: discord.Interaction):
    ...
    try:
        ...
        meta_username = meta_row[0] if meta_row else "Unknown"
        ...
        embed = discord.Embed(
            ...
            description=(
                f"🔩 Nuts: {entry['nuts']}\n"
                f"💎 RP: {entry['rp']} (+{entry['unmined_rp']})\n"
                f"🪙 CC: {entry['cc']} (+{entry['unmined_cc']})\n"
                f"⏳ Token valid: {get_token_seconds_remaining(entry)}s"
            ),
        ...
        await interaction.followup.send("❌ Account lookup failed.", ephemeral=True)
