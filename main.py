...
def save_user_entry(user_id: str, entry: dict, meta_username=None):
    if meta_username:
        cur.execute(
            "INSERT INTO users (user_id, meta_username, data) VALUES (%s, %s, %s) ON CONFLICT (user_id) DO UPDATE SET meta_username = EXCLUDED.meta_username, data = EXCLUDED.data",
            (user_id, meta_username, Json(entry))
        )
    else:
        cur.execute(
            "UPDATE users SET data = %s WHERE user_id = %s",
            (Json(entry), user_id)
        )
    conn.commit()
...
async def connect(interaction: discord.Interaction, meta_username: str):
    ...
    print(f"🔗 {interaction.user} connected as {meta_username}")
...
async def unlink(interaction: discord.Interaction):
    ...
    print(f"🔌 {interaction.user} unlinked")
...
async def account(interaction: discord.Interaction):
    ...
    print(f"📡 Ping from {interaction.user} for /account")
    ...
    if not token_valid:
        class RefreshView(discord.ui.View):
            ...
            async def refresh(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                await interaction.response.send_message("🔄 Token refreshed. Please run `/account` again.", ephemeral=True)
    ...
    embed.description = (
        f"🪪 **| Wallet**\n🔩 Nuts: {nuts}\n💎 RP: {rp}\n🪙 CC: {cc}\n\n"
        f"⛏️ **| Mining Balance**\n💎 RP: {unmined_rp}\n🪙 CC: {unmined_cc}\n\n"
        f"📄 **| Auth Token**\n✅ VALID FOR: {seconds_left} seconds"
    )
    ...
    class AccountView(discord.ui.View):
        ...
        async def claim(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            embed.description = f"+{rp} RP\n+{cc} CC\nUse `/account` again to see updated balance."
...
async def spawnitems(interaction: discord.Interaction, file: discord.Attachment):
    ...
    if not is_valid_token(entry):
        await interaction.followup.send(embed=discord.Embed(
            title="❌ Invalid Token",
            description="Token expired. Use `/account` to refresh.",
            color=discord.Color.red()), ephemeral=True)
        return
    ...
    print(f"📡 Ping from {interaction.user} for /spawnitems")
    ...
    embed.add_field(name="⚠️ Note", value="We are in development — this won’t affect the real game.", inline=False)
